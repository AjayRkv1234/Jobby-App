...
class AboutJob extends Component {
  ...
  getJobsData = async () => {
    ...
    const jobDataDetailsApiUrl = `https://apis.ccbp.in/jobs/${id}`
    const optionsJobData = {
      headers: {Authorization: `Bearer ${jwtToken}`},
      method: 'GET',
    }
    const responseJobData = await fetch(jobDataDetailsApiUrl, optionsJobData)
    if (responseJobData.ok === true) {
      ...
      const updatedSimilarJobsDetails = fetchedData.similar_jobs.map(
        eachItem => ({
          companyLogoUrl: eachItem.company_logo_url,
          id: eachItem.id,
          jobDescription: eachItem.job_description,
  ...
  renderJobFailureView = () => (
    <div className="jo-details-failure-view">
      <img
